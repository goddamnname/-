function H_Filter = GMAP_Filter(H,N,prf,wavelength)
    %% 1加窗
...
%     N = N * 2 - 1;
    %% 3求噪声电平
    %----------------------1、利用零速度点附近为地物，则功率最大，进行此处有无地物判断
%     [Max_spec,Max_indx] = max(real(spec_log));
%     if Max_indx == round(N/2-1) || Max_indx == round(N/2) || Max_indx == round(N/2+1)
    %----------------------结束（未考虑当天气信号强度较强，比地物略高的情况，可能导致未滤除地物增强了该点的天气信号强度）
    %----------------------2、利用零速度点附近为地物，则会出现一定的梯度变化（即功率强度增强），进行此处有无地物判断
%     M_left = spec_log(round(N/2)) - spec_log(round(N/2-1));%分别为中间三个点的强度变化值
%     M_right = spec_log(round(N/2)) - spec_log(round(N/2+1));
%     M_01_left = spec_log(round(N/2-1)) - spec_log(round(N/2-2));
%     M_01_right = spec_log(round(N/2-1)) - spec_log(round(N/2));
%     M_1_left = spec_log(round(N/2+1)) - spec_log(round(N/2+2));
%     M_1_right = spec_log(round(N/2+1)) - spec_log(round(N/2));
%     spec_gate = 3;%梯度判决门限
%     if (M_left > spec_gate || M_right > spec_gate) || (M_01_left > spec_gate || M_01_right > spec_gate) || (M_1_left > spec_gate || M_1_right > spec_gate)
...
%        H_Filter = spec_log();%仅用于观察频谱输出
       ...
       spec_log = ifft(ifftshift(spec_log));%傅里叶逆变换，便于后面PPP计算
       H_Filter = spec_log();
